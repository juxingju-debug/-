...
const App: React.FC = () => {
    ...
    const [fontStyleSourceImage, setFontStyleSourceImage] = useState<File[]>([]); // New State
    ...
    useEffect(() => {
        ...
        const handleWheel = (e: WheelEvent) => {
            ...
            // Calculate world coordinates (relative to the content) under the mouse
            // current_screen_pos = current_offset + world_pos * current_scale
            // world_pos = (current_screen_pos - current_offset) / current_scale
            const worldX = (mouseX - position.x) / scale;
            const worldY = (mouseY - position.y) / scale;

            // Calculate new offset so that world_pos remains at mouseX, mouseY
            // new_offset = current_screen_pos - world_pos * new_scale
    ...
    // Note: Node width is 400px for input/settings, 550px for results.
    // Assuming standard heights, we approximate centers or calculate dynamically if we used refs.
    // For simplicity with this absolute layout, we approximate connection points relative to top-left.
    const inputOut = { x: nodePositions.input.x + 400, y: nodePositions.input.y + 70 }; // Right side
    const settingsIn = { x: nodePositions.settings.x, y: nodePositions.settings.y + 70 }; // Left side
    const settingsOut = { x: nodePositions.settings.x + 400, y: nodePositions.settings.y + 70 }; // Right side
    const resultsIn = { x: nodePositions.results.x, y: nodePositions.results.y + 70 }; // Left side
